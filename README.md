...
- Select **Demo data** to download an SNP sample report for every Saturday in the selected date range. SNP demo records are generated on demand in the browser, so no seeding or database writes are needed. The filename, workbook title, and every worksheet's status column identify the sample data as DEMO. The existing seed/remove controls still manage stored Cool Down and Donations demo records.
